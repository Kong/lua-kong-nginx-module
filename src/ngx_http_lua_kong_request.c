...
static ngx_str_t *search_known_header(ngx_http_request_t *r, ngx_str_t name) {
    ...
    ngx_http_header_t *hh = ngx_hash_find(&cmcf->headers_in_hash, hash, name.data, name.len);
...
static ngx_str_t header_preprocess(ngx_http_request_t *r, ngx_str_t name) {
    ngx_str_t ret;
    ret.data = ngx_palloc(r->pool, name.len);
    ret.len = name.len;

    for (size_t n = 0u; n < name.len; ++n) {
        u_char ch = tolower(name.data[n]);
        if (ch == '-') {
            ch = '_';
        }
        ret.data[n] = ch;
...
// search request header named "name"
// and within search_limit
