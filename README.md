...
Returns `true` when the client sent a request body, `false` otherwise. Works
without a `Content-Length` header, which makes it usable for HTTP/2
...
The answer is based on the number of body bytes actually received, and falls
back to nginx's own "is a body expected" decision while that count cannot yet be
trusted. Only HTTP/2 maintains such a count, so how exact the answer
is depends on the protocol:

| Protocol | Zero length body reported as "no body" |
| --- | --- |
| HTTP/2 | Yes, as soon as the `DATA` frame carrying `END_STREAM` has been parsed |
| HTTP/1.x | No. nginx keeps no count of received body bytes for HTTP/1.x, so a chunked request whose body is only the terminating chunk cannot be told apart from one that still has data coming, and is reported as having a body |
...
frame allowed a body, and the answer is `true`. Calling
